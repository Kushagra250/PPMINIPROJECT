...
public:											// public member functions
	...
	// New member function to deposit money
    ...
    // New member function to change the PIN
	void changePIN(int oldPIN, int newPIN)
	{
	    if (oldPIN == PIN) // Check if the old PIN matches the current PIN
	    {
	        PIN = newPIN; // Update the PIN with the new one
	        cout << endl << "PIN changed successfully.";
	        _getch();
	    }
	    else
	    {
	        cout << endl << "Incorrect old PIN. PIN not changed.";
	        _getch();
	    }
	}
...
int main()
{
	...
	double depositAmount; // Variable to store the deposit amount
	...
	user1.setData(987654321, "Kushagra", 1234, 50000, "9370054900");
	...
	do
	{
		...
		if ((enterAccountNo == user1.getAccountNo()) && (enterPIN == user1.getPIN()))
		{
			do
			{
				...
				case 3: // Add a new case for deposit
                cout << endl << "Enter the deposit amount: ";
                cin >> depositAmount;
                user1.deposit(depositAmount); // Call the deposit function
                break;
				...
				case 6: // Add a new case for changing the PIN
				    int oldPIN, newPIN;
				    cout << endl << "Enter Old PIN: ";
				    cin >> oldPIN;
				    cout << "Enter New PIN: ";
				    cin >> newPIN;
				    user1.changePIN(oldPIN, newPIN);
				    break;
